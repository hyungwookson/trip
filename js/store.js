...
export const store = {
  ...
  async initAuth() {
    ...
    // 로그인 상태를 이 브라우저에 확실히 저장 (사파리에서 세션 유실 방지)
    try { await authMod.setPersistence(auth, authMod.browserLocalPersistence); } catch (e) { console.error("persistence:", e); }
    try {
      const res = await authMod.getRedirectResult(auth);
      if (res && res.user) return res.user;
    } catch (e) { console.error("redirect result:", e); throw e; }
    return null;
  },
  ...
  async signInGoogle() {
    ...
    // 사파리(ITP)는 리다이렉트 복귀 시 인증 상태를 잃어 루프가 생김 → 팝업을 먼저 시도
    try {
  ...
  // ---- 선택지 옮기기 (다른 일정으로) ----
  async moveOption(spotId, planId, fromItemId, optId, toItemId) {
    if (fromItemId === toItemId) return;
    const from = this.item(spotId, planId, fromItemId);
    const to = this.item(spotId, planId, toItemId);
    if (!from || !to) return;
    const idx = from.options.findIndex((o) => o.id === optId);
    if (idx < 0) return;
    const [op] = from.options.splice(idx, 1);         // 원본에서 빼고
    if (from.selectedId === optId) from.selectedId = null;
    (to.options ||= []).push(op);                      // 대상에 그대로 (id 유지 → 투표·대표사진 따라감)
    await this._persist();
  },
  ...
  // ---- 미정(아직 안 정한 장소) 카운트 ----
  planUndecided(plan) { return plan.items.filter((it) => (it.kind === "place" || it.kind === "stay") && !it.selectedId).length; },
  spotTotals(spotId) {
